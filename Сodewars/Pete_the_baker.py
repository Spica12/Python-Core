...
def cakes(recipe, available):
    cakes_available = list()

    for ingridient, amount in recipe.items():

        if  not ingridient in available:
            return 0
        else:
            cakes_available.append(int(available[ingridient] / amount))

    return min(cakes_available)


def cakes_coderwars(recipe, available):
...
print(cakes_coderwars({'flour': 500, 'sugar': 200, 'eggs': 1}, {'flour': 1200, 'sugar': 1200, 'eggs': 5, 'milk': 200}))
print(cakes_coderwars({'apples': 3, 'flour': 300, 'sugar': 150, 'milk': 100, 'oil': 100}, {'sugar': 500, 'flour': 2000, 'milk': 2000}))